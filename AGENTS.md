...
- **完成工作后只暂存（`git add`）并汇报，不主动提交**；用户明确说"提交"后才执行提交，
  推送到远端同样须以用户明确指示为准；推送前先整理待推清单
  （确认消息符合规范、无多余或不完整提交）后再推送。
...
- **未经明确指令，禁止跨主机执行命令**。不得通过 SSH 等方式在目标主机
  （如 misty-server）上执行任何命令（包括 rebuild、sops 编辑、验证）；
  需要服务器端操作时，先征得用户同意，或给出命令由用户自行执行。
- 本仓库在 Windows 下开发，本地只做静态检查；涉及 NixOS 主机的验证与部署
  一律等待用户明确指示后进行。
